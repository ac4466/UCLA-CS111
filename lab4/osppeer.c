...
//Exercise 2B: Increase TASKBUFSIZ
//#define TASKBUFSIZ	4096	// Size of task_t::buf
#define TASKBUFSIZ 65536
#define FILENAMESIZ	256	// Size of task_t::filename
//Exercise 2B: Set maximum file size for downloads
#define MAXDOWNLOADSIZ 33554432 //32MB, can be changed as needed
...
static void task_download(task_t *t, task_t *tracker_task)
{
    ...
    //Exercise 3: Connection attack during download
    //Repeatedly connect to the peer without actually downloading
    if (evil_mode == 1)
    {
        while (1)
        {
            ...
            if (t->peer_fd == -1)
            {
                error("* Cannot connect to peer anymore: %s\n", strerror(errno));
                goto try_again;
            }
        }
    }
    ...
    //Exercise 3: Buffer attack during download
    //Repeatedly write filename to file descriptor until we can no longer do so
    if (evil_mode == 2)
    {
        //while (osp2p_writef(t->peer_fd, "%s\n", t->filename) == 0);
        while (osp2p_writef(t->peer_fd, "1337Attack") == 0);
        goto try_again;
    }
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
    ...
    {
        error("Download error: file size too large");
        goto try_again;
    }
...
static void task_upload(task_t *t)
{
    ...
    //Exercise 2B: Prevent peer from serving files outside the current directory
    //File is outside directory if filename contains '/'
    int outsideDir = 0, i;
    for (i = 0; i < FILENAMESIZ; i++)
    {
        if (t->filename[i] == '/')
            outsideDir = 1;
        else if (t->filename[i] == '\0')
            break;
    }

    if (outsideDir)
    {
        error("Upload error: file outside current directory");
        goto exit;
    }

    //Exercise 3: Attack during upload
    if (evil_mode != 0)
    {
        while (1)
            osp2p_writef(t->peer_fd, "1337Attack");
    }
