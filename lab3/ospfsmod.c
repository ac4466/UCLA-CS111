...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		//r = 1;		/* Fix me! */
		//break;		/* Fix me! */
        uint32_t fpos_offset = OSPFS_DIRENTRY_SIZE * (f_pos - 2);
        if (fpos_offset > dir_oi->oi_size)
        {
            r = 1;
            break;
        }
		...
		/* EXERCISE: Your code here */
        od = ospfs_inode_data(dir_oi, fpos_offset);
...
add_block(ospfs_inode_t *oi)
{
    ...
    if (n_new > OSPFS_MAXFILEBLKS)
        return -ENOSPC;

    int32_t dir_ind = direct_index(n);
    int32_t indir_ind = indir_index(n);
    int32_t indir2_ind = indir2_index(n);

    uint32_t* data_block0 = NULL , *data_block1 = NULL;
    ...
    if (indir_ind == -1)
    {
        ...
        oi->oi_direct[dir_ind] = allocated[0];
        oi->oi_size = n_new * OSPFS_BLKSIZE;
        return 0;
    }
    ...
    if (indir2_ind == 0)
    {
        if (oi->oi_indirect2 == 0)
        {
            ...
            data_block0 = ospfs_block(oi->oi_indirect2);

        if (data_block0[indir_ind] == 0)
        {
    ...
    data_block1 = ospfs_block(allocated[1]);
    data_block1[dir_ind] = allocate_block();
    if (data_block1[dir_ind] == 0)
    {
        free_allocated(allocated);
        return -ENOSPC;
    }
    memset(ospfs_block(data_block1[dir_ind]), 0, OSPFS_BLKSIZE);
    oi->oi_size = n_new * OSPFS_BLKSIZE;
    ...
    if (indir2_ind == 0)
    {
        ...
        if (data_block0[indir_ind] == 0)
            data_block0[indir_ind] = allocated[1];
        return 0;
    }
...
remove_block(ospfs_inode_t *oi)
{
    ...
    int32_t dir_ind = direct_index(n_new);
    int32_t indir_ind = indir_index(n_new);
    int32_t indir2_ind = indir2_index(n_new);
    ...
    if (indir_ind == -1)
    {
        free_block(oi->oi_direct[dir_ind]);
        oi->oi_direct[dir_ind] = 0;
        oi->oi_size = n_new * OSPFS_BLKSIZE;
        return 0;
    }
    if (indir2_ind == 0)
    {
        ...
        data_block0 = ospfs_block(oi->oi_indirect2);
        data_block1 = ospfs_block(data_block0[indir_ind]);

        if (data_block1 == NULL)
            return -EIO;

        free_block(data_block1[dir_ind]);
        data_block1[dir_ind] = 0;
        oi->oi_size = n_new * OSPFS_BLKSIZE;

        if (dir_ind == 0)
        {
            free_block(data_block1[indir_ind]);
            data_block0[indir_ind] = 0;
    ...
    else if (indir2_ind == -1)
    {
        data_block1 = ospfs_block(oi->oi_indirect);
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		/* EXERCISE: Your code here */
        uint32_t pos_offset = *f_pos % OSPFS_BLKSIZE;
        uint32_t data_left = count - amount;

        if (pos_offset + data_left > OSPFS_BLKSIZE)
            n = OSPFS_BLKSIZE - pos_offset;
        else
            n = data_left;

        if (copy_to_user(buffer, data + pos_offset, n) > 0)
        {
            retval = -EFAULT;
            goto done;
        }
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
        ...
        if (copy_from_user(data + pos_offset, buffer, n) > 0)
        {
            retval = -EFAULT;
            goto done;
        }
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
    ...
    while (dir_offset < dir_oi->oi_size)
    {
        od = ospfs_inode_data(dir_oi, dir_offset);
        if (od->od_ino == 0)
            return od;
        od = NULL;
        dir_offset += OSPFS_DIRENTRY_SIZE;
    }
    ...
    return ospfs_inode_data(dir_oi, dir_offset);
}
...
get_new_inode(void)
{
    ...
    uint32_t new_inode_no = 2;
    while (new_inode_no < ospfs_super->os_ninodes)
    {
        new_inode = ospfs_inode(new_inode_no);
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
    ...
    if (entry_ino == 0)
        return -ENOSPC;
    new_sym_inode = ospfs_inode(entry_ino);
    ...
    memcpy(new_sym_inode->oi_symlink, symname, new_sym_inode->oi_size);
    new_sym_inode->oi_symlink[new_sym_inode->oi_size] = 0;
